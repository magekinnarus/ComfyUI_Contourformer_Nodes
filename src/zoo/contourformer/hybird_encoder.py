...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, key_padding_mask=None,pos_embed=None) -> torch.Tensor:
        residual = src
        if self.normalize_before:
            src = self.norm1(src)
        q = k = self.with_pos_embed(src, pos_embed)
        src, _ = self.self_attn(q, k, value=src, attn_mask=src_mask,key_padding_mask=key_padding_mask)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, src_mask=None, key_padding_mask=None,pos_embed=None) -> torch.Tensor:
        ...
        for layer in self.layers:
            output = layer(output, src_mask=src_mask, pos_embed=pos_embed,key_padding_mask=key_padding_mask)
