...
def distance2point(points, distance, reg_scale):
    """
    Decodes edge-distances into bounding box coordinates.

    Args:
        points (Tensor): (B, N, x, 2).
        distance (Tensor): (B, N, x, 2)
    ...
    reg_scale = abs(reg_scale)
    x = points[..., 0] + (0.5 * reg_scale + distance[..., 0]) * (w / reg_scale)
    y = points[..., 1] + (0.5 * reg_scale + distance[..., 1]) * (h / reg_scale)

    new_points = torch.stack([x,y], -1) #
