...
@register()
class ContourHungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0):
        ...
        self.cost_pts = weight_dict['cost_pts']
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False):
        ...
        if "pred_coords" in outputs:
            ...
            tgt_pts = torch.cat([v["instance_polys"] for v in targets]) # [num_queries, 128, 2]

            # Compute the L1 cost between points
            cost_pts = torch.cdist(out_pts.flatten(1), tgt_pts.flatten(1), p=1)
            # Final cost matrix 3 * self.cost_bbox + 2 * self.cost_class + self.cost_giou
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou + self.cost_pts * cost_pts
        else:
